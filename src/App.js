import React, { useState, useEffect } from "react";
import PropTypes from "prop-types";
import { BrowserRouter as Router, Route, Routes, Link } from "react-router-dom";
import "./App.css";
import Login from "./login";
import img1 from "./images/customer1.jpeg";
// import Navbar from "./Navbar";
// import HeroSection from "./HeroSection";
// import HomePage from "./HomePage";
// import Footer from "./Footer";
import Register from "./Register"; // Import the Register component

function Navbar() {
  return (
    <nav>
      <Link to="/">Home</Link>
      <Link to="/login">Login</Link>
      <Link to="/register">Register</Link>
      <Link to="/contact">Contact Us</Link>
    </nav>
  );
}

function HeroSection() {
  return (
    <div className="hero-section">
      <div className="hero-content">
        <h1>Welcome to Our Transport Services</h1>
        <p>Your trusted partner for safe and reliable transportation.</p>
        <Link to="/book-now" className="btn-book-now">
          Book Now
        </Link>
      </div>
    </div>
  );
}

function Home() {
  return (
    <div>
      <HeroSection />
      <HomePage />
    </div>
  );
}

function HomePage() {
  const [carouselIndex, setCarouselIndex] = useState(0);
  const carouselImages = [
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWa1rJSYqNIgbpLi8iRHFW8QUwo1ubHLLY3Q&usqp=CAU",
    "g",
    "https://assets.volvo.com/is/image/VolvoInformationTechnologyAB/BS%20VI%20Bus%20Single%20Axcel_Single%20Bus?wid=1400&fit=constrain",
    "https://static.tnn.in/thumb/msid-94358056,updatedat-1663775713071,width-1280,height-720,resizemode-75/94358056.jpg",
    "https://media.istockphoto.com/id/135327019/photo/white-passenger-bus.jpg?s=612x612&w=0&k=20&c=sRU5BpOsY6fyYj9pMiAxz0dLML--uoNl7rIXelRbFnc=",
  ];

  useEffect(() => {
    const interval = setInterval(() => {
      setCarouselIndex((prevIndex) => (prevIndex + 1) % carouselImages.length);
    }, 5000);

    return () => clearInterval(interval);
  }, []);

  return (
    <div className="homepage">
      <Carousel images={carouselImages} activeIndex={carouselIndex} />
      <BookNowSection />
      <CustomerReviewsSection />
    </div>
  );
}

function Carousel({ images, activeIndex }) {
  return (
    <div className="carousel">
      {images.map((image, index) => (
        <img
          key={index}
          src={image}
          alt={`Image ${index}`}
          className={index === activeIndex ? "active" : ""}
        />
      ))}
    </div>
  );
}
Carousel.propTypes = {
  images: PropTypes.arrayOf(PropTypes.string).isRequired,
  activeIndex: PropTypes.number.isRequired,
};

function BookNowSection() {
  return (
    <div className="book-now">
      <p>

      </p>
       <button className="book-now-button">Book Now</button>
    </div>
  );
}

function CustomerReviewsSection() {
  const reviews = [
    {
      text: "Awesome service! Will use again.",
      rating: 4.5,
      imageUrl: "./images/customer1.jpeg",
    },
    {
      text: "The best transport operator in town!",
      rating: 4.5,
      imageUrl: "./images/customer2.jpeg",
    },
    {
      text: "Professional and reliable service.",
      rating: 5,
      imageUrl: "./images/customer3.jpeg",
    },
    {
      text: "Smooth and comfortable rides every time.",
      rating: 4,
      imageUrl: "./images/customer4.jpeg",
    },
    {
      text: "Highly recommended transport service.",
      rating: 4.5,
      imageUrl: "./images/customer5.jpeg",
    },
    {
      text: "Exceptional experience every time.",
      rating: 4.5,
      imageUrl: "./images/customer6.jpeg",
    },
  ];

  return (
    <div className="customer-reviews">
      <h2>Customer Reviews</h2>
      <div className="review-grid">
        {reviews.map((review, index) => (
          <div className="review-box" key={index}>
            <img
              // src={require(`${review.imageUrl}`).default}
              src="data:image/png;base64,"
              alt={`Customer ${index + 1}`}
            />
            <p>{review.text}</p>
            <div className="star-rating">
              {review.rating} <span className="star-icon">★</span>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}

function Footer() {
  return <footer>{/* Your footer content */}</footer>;
}

function App() {
  return (
    <Router>
      <div>
        <Navbar />
        <Routes>
          <Route path="/" element={<Home />} />
          <Route path="/login" element={<Login />} />
          <Route path="/register" element={<Register />} />{" "}
          
          {/* Add this line */}
        </Routes>
        <Footer />
      </div>
    </Router>
  );
}

export default App;
